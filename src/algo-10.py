...
@algo_player(name="Liuri Loami 9 - Ultimate",
...
def liuriloami(board, game_id, player_id):
    ...
    for tile_id, tile in board.items():
        ...
        if (tile["player"] is None and neighbours < bestCorner and vul > 3):
            corner = True
        ...
        if (tile["player"] == player_id):
            ...
            if (isTileSafe(tile_id, player_id, board) == True):
                continue
   ...
   # If not, return a random tile
    for tile_id, tile in board.items():
        if (tile["player"] == player_id or tile["player"] is None):
            return tile_id
